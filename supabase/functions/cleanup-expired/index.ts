...
Deno.serve(async req => {
  ...
  const {data:tasks,error}=await service.from("video_tasks").select("id,source_url,output_storage_path,narration_storage_path").lt("expires_at",now).neq("status","expired");
  ...
  for(const task of tasks||[]){
    ...
    if(task.narration_storage_path?.startsWith("storage://")){const location=task.narration_storage_path.slice(10);const slash=location.indexOf("/");await service.storage.from(location.slice(0,slash)).remove([location.slice(slash+1)]);}
    if(task.output_storage_path)await service.storage.from("video-processed").remove([task.output_storage_path]);
    await service.from("video_tasks").update({status:"expired",current_step:"Files expired and deleted",processed_file_url:null,output_storage_path:null,narration_storage_path:null}).eq("id",task.id);cleaned++;
  }
