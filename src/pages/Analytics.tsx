...
import { BarChart3, CheckCircle, Clock, Target, TrendingUp, XCircle } from 'lucide-react';
...
export default function Analytics() {
  ...
  return <div className="min-h-screen py-8"><div className="container mx-auto px-4 max-w-6xl">
    <h1 className="text-4xl font-bold text-brass-200 mb-2">Analytics</h1><p className="text-brass-300 mb-8">Live statistics calculated from your recap processing records.</p>
    {error&&<div role="alert" className="p-4 rounded bg-red-950/40 text-red-300 mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"><span>{error}</span><button onClick={()=>setReloadToken(value=>value+1)} className="underline hover:text-white self-start sm:self-auto">Retry</button></div>}
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">{cards.map(({label,value,icon:Icon})=><div key={label} className="steampunk-card p-5"><Icon className="w-5 h-5 text-brass-400 mb-3"/><div className="text-2xl font-bold text-brass-100">{loading?'…':isAvailable?value:'—'}</div><div className="text-sm text-brass-400">{label}</div></div>)}</div>
